Feature: Kafka Test Suite3
  ...
  Scenario: Creating Consumer
    ...
    Then status 200
    * print response
    * match response.instance_id == "dhanaraju_consumer"
  ...
  Scenario: Subscribing to topic
    Given path "/consumers/my-consumer-group/instances/dhanaraju_consumer/subscription"
    And request
      """
      {
        "topics" : [ "dhana_topic" ]
  ...
  Scenario: Getting Subscription
    ...
    Then status 200
    * print response
    * assert response.topics[0] == "dhana_topic"


  Scenario: Getting Subscription records
    Given path "/consumers/my-consumer-group/instances/dhanaraju_consumer/records"
  ...
  Scenario: Getting assigned Assignments
    Given path "/consumers/my-consumer-group/instances/dhanaraju_consumer/assignments"
    And header Accept = "application/vnd.kafka.v2+json"
    When method get
    Then status 200
    * print response
    * assert response.partitions[0].topic == "dhana_topic"
  ...
  Scenario: Deleting consumer
    Given path "/consumers/my-consumer-group/instances/dhanaraju_consumer"
    And header Accept = "application/vnd.kafka.v2+json"
    When method delete
    Then status 204
